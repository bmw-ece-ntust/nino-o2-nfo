...
class KubernetesDeploymentSerializer(serializers.ModelSerializer):
    ...
    cluster_name = serializers.CharField(source='k8s_cluster.cluster.name', read_only=True)
...
class DeploymentDescriptorDetailedSerializer(serializers.ModelSerializer):
    ...
    target_cluster_name = serializers.CharField(source='target_cluster.name', read_only=True)
    target_cluster_status = serializers.CharField(source='target_cluster.status', read_only=True)
...
class NfDeploymentInstanceDetailedSerializer(serializers.ModelSerializer):
    ...
    descriptor_name = serializers.CharField(source='descriptor.name', read_only=True)
    descriptor_profile = serializers.CharField(source='descriptor.profile_type', read_only=True)
    cluster_name = serializers.CharField(source='deployed_cluster.name', read_only=True)
    cluster_status = serializers.CharField(source='deployed_cluster.status', read_only=True)
...
class VnfLcmOperationDetailedSerializer(serializers.ModelSerializer):
    ...
    vnf_instance_name = serializers.CharField(source='vnf_instance.name', read_only=True)
    vnf_instance_state = serializers.CharField(source='vnf_instance.instantiation_state', read_only=True)
...
class KubernetesDeploymentDetailedSerializer(serializers.ModelSerializer):
    ...
    nf_instance_name = serializers.CharField(source='nf_instance.name', read_only=True)
    cluster_name = serializers.CharField(source='k8s_cluster.cluster.name', read_only=True)
    cluster_endpoint = serializers.CharField(source='k8s_cluster.api_endpoint', read_only=True)
