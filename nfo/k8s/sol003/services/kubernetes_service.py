...
class KubernetesDeploymentService:
    ...
    def delete_helm_release(self, namespace, release_name, force=False):
        ...
        with self._get_kubeconfig_context() as env:
            try:
                ...
                list_cmd = ['helm', 'list', '-n', namespace, '--all','-o', 'json']
    ...
    def upgrade_helm_release(self, namespace, release_name, chart_repo, chart_path, values=None):
        ...
        with self._get_kubeconfig_context() as env:
            try:
                # Check if release exists
                check_cmd = ['helm', 'list', '-n', namespace, '--all','-o', 'json']
    ...
    def _upgrade_from_git(self, namespace, release_name, repo_url, chart_path, values=None, env=None):
        ...
        try:
            ...
            # Prepare values
            values_files = self._prepare_helm_values(work_dir, chart_path, values, temp_values_files)
    ...
    def deploy_helm_chart(self, namespace, release_name, chart_repo, chart_path, chart_branch, values=None):
        """Deploy Helm chart from any source"""
        with self._get_kubeconfig_context() as env:
            if 'git' in chart_repo:
                return self._deploy_from_git(namespace, release_name, chart_repo, chart_path, chart_branch, values, env)
            else:
                return self._deploy_from_helm_repo(namespace, release_name, chart_repo, chart_path, values, env)

    def _deploy_from_git(self, namespace, release_name, repo_url, chart_path, chart_branch, values=None, env=None):
        ...
        try:
            ...
            clone_cmd = ['git', 'clone', '--depth', '1', '--branch', chart_branch, repo_url, work_dir]
            ...
            if not dry_run_result['success']:
                ...
                return {'success': False, 'error': f"Dry run failed: {dry_run_result['error']}"}
            ...
            print(f"[DEBUG]: Install Command {install_cmd}")
            ...
            if result['success']:
                ...
                if "provided port" in result['error']:
                    logger.warning(f"Port conflict for {release_name}, but deployment may have succeeded")
                    return {'success': True, 'message': f'Deployed {release_name} with port warning'}
    ...
    def _upgrade_from_helm_repo(self, namespace, release_name, repo_url, chart_name, values=None, env=None):
        ...
        helm_cmd = [
            'helm', 'upgrade', release_name, f"{repo_name}/{chart_name}",
            '--namespace', namespace,
            '--wait',
            '--timeout', '10m'
        ]
            ...
            if helm_release:
                with self._get_kubeconfig_context() as env:
                    ...
                    if result['success']:
                        ...
                        logger.error(f"Failed to delete Helm release: {result['error']}")
                        return False
            else:
                apps_v1 = client.AppsV1Api(self.k8s_client)
                apps_v1.delete_namespaced_deployment(
