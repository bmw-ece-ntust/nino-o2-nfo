...
class DeploymentDescriptor(models.Model):
    ...
    target_cluster = models.ForeignKey('KubernetesCluster', on_delete=models.CASCADE, related_name='deployment_descriptors', null=True)
...
class NfDeploymentInstance(models.Model):
    ...
    descriptor = models.ForeignKey(DeploymentDescriptor, on_delete=models.CASCADE, related_name='instances')
    name = models.CharField(max_length=200)
    user_provided_namespace = models.CharField(max_length=255, null=True, blank=True)
    # State management
...
class VnfLcmOperation(models.Model):
    ...
    vnf_instance = models.ForeignKey(NfDeploymentInstance, on_delete=models.CASCADE, related_name='operations')
    ...
    state_snapshot = models.JSONField(default=dict, blank=True)  # State before operation
...
class KubernetesCluster(models.Model):
    ...
    api_endpoint = models.URLField(help_text="Kubernetes API server endpoint", unique=True)
    ...
    auth_method = models.CharField(max_length=20, choices=AUTH_METHODS, default='kubeconfig')

    # Encrypted credentials storage
    kubeconfig_data = models.TextField(blank=True, help_text="Base64 encoded kubeconfig file")
    ...
    def set_kubeconfig(self, kubeconfig_content):
        ...
        if isinstance(kubeconfig_content, str):
            self.kubeconfig_data = base64.b64encode(kubeconfig_content.encode()).decode()
        else:
...
class KubernetesDeployment(models.Model):
    ...
    nf_instance = models.OneToOneField('NfDeploymentInstance', on_delete=models.CASCADE, related_name='k8s_deployment')
    k8s_cluster = models.ForeignKey(KubernetesCluster, on_delete=models.CASCADE, related_name='deployments')
